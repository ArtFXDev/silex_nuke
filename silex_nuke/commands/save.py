...
from silex_nuke.utils.utils import Utils
...
from silex_nuke.utils.dialogs import Dialogs
...
class Save(CommandBase):
    ...
    async def __call__(self, upstream: Any, parameters: Dict[str, Any], action_query: ActionQuery):
        ...
        async def get_scene_path():
            ...
            soft = await gazu.files.get_software_by_name("nuke")
        ...
        def save():
            ...
            logger.info('comp saved to : {}'.format(
                working_file_with_extension))

            nuke.scriptSaveAs(working_file_with_extension)
